...
import cairo
import sprites
import orbit_effects
from screensaver_art import Screensaver
from space_pose import SPACE_SCALE,SPACE_ANCHOR
import combat_fx as fx
from wyrm_scene import ground as mist_ground, fog as mist_fog, lightning as mist_lightning
from laser_art import draw_arc_beam
from finale import REAR_VIEW_AT,ENDING_DURATION,DEPARTURE_LAUNCH_AT,boarding_pose,ENCOUNTER_DURATION
from battle_art import color,glow,line,label
...
class FinaleRenderer:
    ...
    def wheels(self,c,x,y,travel=None):
        # Spin only the alloy rims. Tire silhouettes and axles remain attached
        # to the body; rotation follows distance, stopping when the car stops.
        angle=((x+430) if travel is None else travel)/34
        for sx,sy in ((560,836),(942,838)):
            px=x+(sx-450)*430/640;py=y+(sy-680)*155/230
            # Existing tire, brake caliper and rim outline remain untouched.
            c.save();c.translate(px,py);c.scale(.9,1)
            for i in range(5):
                a=angle+i*math.tau/5
                line(c,[(math.cos(a)*7,math.sin(a)*7),(math.cos(a)*20,math.sin(a)*20)],'cream',1,.35)
            c.restore()
