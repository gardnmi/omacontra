...
from intro_music import IntroMusic,TRACK,START_SOUND,GAME_TRACKS

class IntroMusicTests(unittest.TestCase):
    def test_only_starts_for_visible_opening_and_stops_after_start(self):
        process=Mock();process.poll.return_value=None
        with patch('intro_music.subprocess.Popen',return_value=process) as launch:
    ...
    def test_playlist_continues_across_story_levels_death_and_retry(self):
        ...
        def tick(active):
            with patch('boss_app.time.monotonic',return_value=app.last+.02):app.tick()
    ...
    def test_missing_player_does_not_break_game_or_retry_every_tick(self):
        with patch('intro_music.subprocess.Popen',side_effect=FileNotFoundError),patch('builtins.print'):
