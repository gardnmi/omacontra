...
def stream_frames(floor=False):
    ...
    for tick in range(16):
        ...
        c=cairo.Context(surface);c.set_operator(cairo.OPERATOR_ADD)
        for pose,weight in ((poses[index],1-mix),(poses[(index+1)%4],mix)):
            frame=(pose*512+5,708,500,213) if floor else (pose*512+12,176,490,240)
...
def fire_floor(c,t,strength):
    if strength<=0:return
    frames=stream_frames(True);height=68*strength
    for i in range(4):
        c.save();c.translate(i*320-3,635-height);c.scale(327/384,height/192)
        c.set_source_surface(frames[(int(t*24)+i*5)%16])
...
def cabinet(c,opened=False):
    if opened:
        sprites.draw(c,PROPS,(1040,40,378,491),115,550,98,80)
    else:
        sprites.draw(c,PROPS,(688,40,214,491),159,550,55,80)
