...
@lru_cache(maxsize=1)
def wisp_stamp():
    s=cairo.ImageSurface(cairo.FORMAT_ARGB32,384,128)
    sprites.draw(cairo.Context(s),'wyrm-storm-wisp.png',(0,250,1536,550),0,0,384,128)
    return s

@lru_cache(maxsize=1)
def eye_comet_stamp():
    s=cairo.ImageSurface(cairo.FORMAT_ARGB32,160,64)
    sprites.draw(cairo.Context(s),'wyrm-eye-comet.png',(40,270,1440,530),0,0,160,64)
    return s

def eye_comet(c,x,y,angle,age):
    c.save();c.translate(x,y);c.rotate(angle)
    # Bright head, rather than the full decorative tail, is the hitbox center.
    c.translate(-61,-14);c.scale(.45,.45)
    c.set_source_surface(eye_comet_stamp());c.paint();c.restore()
    halo(c,x,y,10,.3+.15*math.sin(age*19))
...
def wisp(c,x,y,angle,length=95,width=33,alpha=1):
    if min(length,width,alpha)<=0:return
    c.save();c.translate(x,y);c.rotate(angle)
    c.translate(-length+8,-width*.56);c.scale(length/384,width/128)
    c.set_source_surface(wisp_stamp());c.paint_with_alpha(alpha);c.restore()

def breath(c,sx,sy,ex,ey,t,alpha=1,width=48):
    angle=math.atan2(ey-sy,ex-sx);length=math.hypot(ex-sx,ey-sy)
    # Stream grows from the jaw, with a visible pressure front and dark edges.
    for i in range(3):
        reach=length*(.94+.03*math.sin(t*8+i*2))
        wisp(c,sx+math.cos(angle)*reach,sy+math.sin(angle)*reach,angle,
             reach,width*(.82+i*.14),alpha*(.65 if i==0 else .24))
