...
- `story.py`, `art.py`: cinematic timeline and drawing.
- `combat.py`: reaper simulation, independent of GTK/Hyprland.
- `chase.py`, `chase_art.py`: Quattro simulation and rendering.
- `chase_cinema.py`: opening and victory panel framing; presentation only.
- `battle_art.py`: boss, character, and effects rendering.
- `boss_app.py`: fullscreen viewport, single-window lifecycle, input and cleanup.
...
`tidebreaker.py` owns the encounter, `tide_art.py` owns its rendering, and
`journey_cinema.py` owns the harbor/crossing cinematics. Tidebreaker reuses DHH's
...
Guardian combat is in `tide_guardians.py`; poses, world changes, and reveal
framing are in `tide_guardian_art.py`. The sprites preserve the supplied
...
Simulation: `tide_cargo.py`; artwork: `tide_cargo_art.py`.
...
Simulation events in `combat.py` drive sound once per action. Impacts are throttled;
invulnerability does not repeatedly play hurt sounds. These hooks apply only to level 1.

`reaper_audio.py` mixes at most six voices into the existing machine-gun SDL stream.
...
`reaper_environment.py` animates existing fabric artwork, red caged fixtures,
...
and the planted locking thud at 2.5 s, matching `chase_robot_art.py`.
...
`chase_audio.py` configures the shared six-voice mixer; `Chase.sound` and
`chase_robot.py` supply simulation events without changing combat timing.
...
`chase_environment.py` adds independent drifting cloud wisps, sunset reflection ripples,
...
Final-level combat now uses `finale_audio.py` on the shared SDL device. Ring,
...
Only Earth-phase gameplay enables `orbit_effects.py`: star shimmer, a passing
