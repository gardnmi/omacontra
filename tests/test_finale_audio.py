...
from omacontra.audio.finale_audio import FinaleEffects, AUDIO
...
class FinaleAudioTests(unittest.TestCase):
    ...
    def test_bank_loads_all_variants_and_routes_to_shared_device(self):
        bank=FinaleEffects()
        paths=list(AUDIO.glob('*.wav'));self.assertEqual(len(paths),16)
        for path in paths:
    ...
    def test_orbital_effects_only_run_in_first_phase(self):
        ...
        with patch('omacontra.stages.space.orbit_effects.draw') as effects:
