...
class ChaseTests(unittest.TestCase):
    def test_control_playthrough_completes_both_phases(self):
        from tools.chase_playtest import run
        f,trace=run()
    ...
    def test_robot_shredder_has_a_normal_jump_escape(self):
        from omacontra.stages.highway.chase_robot import robot_step
        for jump in (False,True):
    ...
    def test_robot_attacks_and_heart_repeat(self):
        from omacontra.stages.highway.chase_robot import heart_open
        f=Chase();f.detach_trailer()
        for _ in range(86):f.step(.04)
    ...
    def test_destroyed_cab_does_not_draw_detached_trailer_again(self):
        import cairo; from omacontra.rendering import sprites
        ...
        with patch('omacontra.stages.highway.chase_art.sprites.draw',wraps=sprites.draw) as draw:
