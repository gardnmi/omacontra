...
from wyrm_scene import MOUTH,EYES,point,pose,head_angle,lightning_paths
...
class WyrmSceneTests(unittest.TestCase):
    def test_final_charge_lights_three_sources_without_firing(self):
        ...
        with patch('wyrm_scene.breath') as blast,patch('wyrm_scene.storm_lance') as lance,patch('wyrm_scene.halo',wraps=wyrm_scene.halo) as halo:
            r.draw(c,f)
            blast.assert_not_called();lance.assert_not_called()
    ...
    def test_rescue_and_boarding_keep_new_scenery_and_original_car(self):
        ...
        with patch('sprites.draw',wraps=sprites.draw) as draw:
            renderer.draw(c,f)
