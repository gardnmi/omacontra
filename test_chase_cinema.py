...
from chase_cinema import ChaseCinema, INTRO_DURATION, PANEL
...
class CinemaTests(unittest.TestCase):
    ...
    def test_host_freezes_combat_and_enters_outro_once(self):
        ...
        with patch('boss_app.time.monotonic',return_value=.02):app.tick()
        self.assertEqual(app.f.clock,0);self.assertGreater(app.chase_cinema.age,0)
        app.chase_cinema.skip()
        with patch('boss_app.time.monotonic',return_value=.04):app.tick()
        self.assertIsNone(app.chase_cinema)
        app.f.state='won'
        with patch('boss_app.time.monotonic',return_value=.06):app.tick()
        self.assertEqual(app.chase_cinema.kind,'outro');cinema=app.chase_cinema
        with patch('boss_app.time.monotonic',return_value=.08):app.tick()
